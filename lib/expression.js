...
CronExpression.constraints = [
  ...
  { min: 1, max: 31, chars: ['L', 'W'] }, // Day of month
...
// TODO: add w as a valid character
CronExpression.standardValidCharacters = /^[,*\d/-]+$/;
CronExpression.dayOfWeekValidCharacters = /^[?,*\dL#/-]+$/;
CronExpression.dayOfMonthValidCharacters = /^[?,*\dWL/-]+$/;
  ...
  if (mappedFields.month.length === 1) {
    ...
    return mappedFields.dayOfMonth
      .filter(function(dayOfMonth) {
        return ['L','LW'].includes(dayOfMonth) ? true : dayOfMonth <= daysInMonth;
      })
...
CronExpression.prototype._findSchedule = function _findSchedule (reverse) {
  ...
  function isWInExpressions(expressions) {
    return expressions.length > 0 && expressions.some(function(expression) {
      return typeof expression === 'string' && expression.indexOf('W') >= 0;
    });
  }

  function isLastWorkingDayOfMonth(expression) {
      // There might be multiple expressions and not all of them will contain
      // the "L".
      if (!isLInExpressions(expression) || !isWInExpressions(expression)) {
        return false;
      }
      var weekday = currentDate.getDay();
      var holidays = [CronExpression.aliases.dayOfWeek.sun, CronExpression.aliases.dayOfWeek.sat];
      if(holidays.includes(weekday)) return false; 
      var daysInMonth = CronExpression.daysInMonth[currentDate.getMonth()];
      if(checkFebruaryAndNotLeapYear(currentDate)) daysInMonth = 28 //Added to fix "0 0 31 * ?" For february to return 29 for leap year and 28 for other years 
      if (daysInMonth - currentDate.getDate() > holidays.length) return false;
      var processingDate = currentDate.getDate();
      while (processingDate < daysInMonth){
        processingDate += 1;
        var newDate = new Date(currentDate.getFullYear(), currentDate.getMonth(), processingDate);
        if(!holidays.includes(newDate.getDay())) return false;
      }
      return true;
  }

  function isNearestWorkingDayOfMonth(expression) {
    if (!isWInExpressions(expression)) {
      return false;
    }
    var monthDate = Number.parseInt(expression[0]);
    if(expression[0] === 'W'){
      monthDate = 1;
    }
    if (Number.isNaN(monthDate)) {
      throw new Error('Invalid date of the month expression: ' + expression);
    }
    if (monthDate > 7) {
      throw new Error('Invalid date of the month expression W: ' + expression);
    }
    
    var holidays = [CronExpression.aliases.dayOfWeek.sun, CronExpression.aliases.dayOfWeek.sat];
    if(CronExpression.daysInMonth[currentDate.getMonth()] < monthDate) return false;
    if(holidays.includes(currentDate.getDay())) return false;
    var dateDiff = currentDate.getDate() - monthDate;
    if(dateDiff === 0) return true;
    if((dateDiff === -1 ) && currentDate.getDay() === CronExpression.aliases.dayOfWeek.fri){
      return true;
    }
    if((dateDiff === 1 || dateDiff === 2) && currentDate.getDay() === CronExpression.aliases.dayOfWeek.mon){
      return true; 
    }
    return false;
}
  ...
  if(!this._hasIterated){
    if(this._options.includeCurrentDate && !reverse){
      this._currentDate.subtractDay();
    } else if(!this._options.includeCurrentDate && reverse){
      this._currentDate.subtractDay();
    }
  }
  ...
  while (stepCount < LOOP_LIMIT) {
    ...
    var dayOfMonth = this.fields.dayOfMonth;

    var dayOfMonthMatch = matchSchedule(currentDate.getDate(), dayOfMonth);
    if (isLInExpressions(dayOfMonth) && isWInExpressions(dayOfMonth)) {
      dayOfMonthMatch = dayOfMonthMatch || isLastWorkingDayOfMonth(dayOfMonth);
    }else if (isWInExpressions(dayOfMonth)) {
      dayOfMonthMatch = dayOfMonthMatch || isNearestWorkingDayOfMonth(dayOfMonth);
    }else if (isLInExpressions(dayOfMonth)) {
      dayOfMonthMatch = dayOfMonthMatch || currentDate.isLastDayOfMonth();
    }else if(
      CronExpression.daysInMonth[currentDate.getMonth()] <= Number(dayOfMonth) &&
      (
        CronExpression.daysInMonth[currentDate.getMonth()] === currentDate.getDate() || 
        checkFebruaryAndNotLeapYear(currentDate) && 28 === currentDate.getDate()  // Added to fix "0 0 31 * ?" For february to return 29 for leap year and 28 for other years 
      )
    ){
...
CronExpression.prototype._findScheduleWithFrequency = function _findScheduleWithFrequency (reverse) {
  var hasIterated = this._hasIterated;
  var lastDate = this._currentDate;
  var frequency = this._options.frequency;
  var frequencyType = this._options.frequencyType;
  var schedule = this._findSchedule(reverse);
  var dayOfMonth = this.fields.dayOfMonth[0];
  if((frequencyType !== 'yearly' && frequencyType !== 'monthly') && typeof dayOfMonth === 'string' && (dayOfMonth.includes('W') || dayOfMonth.includes('L'))){
      throw new Error('Invalid expression, W/L with non monthly frequency type');
  }
  if(hasIterated && frequency && lastDate ){
    var stepCount = 0;
    if(frequencyType === 'weekly'){
      var weekDiff = schedule.getWeekDiff(lastDate);
      while( Math.abs(weekDiff) !== frequency && weekDiff !== 0 && stepCount < FREQ_LIMIT ) {
        schedule = this._findSchedule(reverse);
        weekDiff = schedule.getWeekDiff(lastDate);
        stepCount += 1;
      }
    }else if(frequencyType === 'monthly'){
      var monthDiff = schedule.getMonthDiff(lastDate);
      while( Math.abs(monthDiff) !== frequency && monthDiff !== 0 && stepCount < FREQ_LIMIT) {
        schedule = this._findSchedule(reverse);
        monthDiff = schedule.getMonthDiff(lastDate);
        stepCount += 1;
      }
    }else if(frequencyType === 'yearly'){
      var yearDiff = schedule.getYearDiff(lastDate);
      while( Math.abs(yearDiff) !== frequency && yearDiff !== 0 && stepCount < FREQ_LIMIT) {
        schedule = this._findSchedule(reverse);
        yearDiff = schedule.getYearDiff(lastDate);
        stepCount += 1;
      }
    }
    if (stepCount >= FREQ_LIMIT) {
      throw new Error('Invalid expression, frequency loop limit exceeded');
    }
  }
  return schedule;
};
...
CronExpression.prototype.next = function next () {
  var schedule = this._findScheduleWithFrequency();
...
CronExpression.prototype.prev = function prev () {
  var schedule = this._findScheduleWithFrequency(true);
...
CronExpression.prototype.hasNext = function() {
  ...
  try {
    this._findScheduleWithFrequency();
    return true;
  } catch (err) {
    return false;
  } finally {
...
CronExpression.prototype.hasPrev = function() {
  ...
  try {
    this._findScheduleWithFrequency(true);
    return true;
  } catch (err) {
    return false;
  } finally {
...
function checkFebruaryAndNotLeapYear(date) {
  //Added to fix "0 0 31 * ?" For february to return 29 for leap year and 28 for other years 
  const year = date.getFullYear();
  const month = date.getMonth();
  const isFebruary = (month === 1);
  const isLeapYear = (year % 4 === 0 && (year % 100 !== 0 || year % 400 === 0));
  return isFebruary && !isLeapYear
}
