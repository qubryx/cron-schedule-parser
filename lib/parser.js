    ...
    next: () => {
      ...
      if(lastDate >= endDate){
        if(parentParser.hasNext()){
          ...
          endDate = getNewEndDate(expression, options, endDate)
        } 
...
function convertToTimeZone1(date, tz) {
  if(!tz){
    const { timeZone } = Intl.DateTimeFormat().resolvedOptions();
    tz = timeZone
  }
  ...
  // return DateTime.fromJSDate(new Date(date)).setZone(tz).toJSDate();
}
function getNextDate1(date, type, timezone) {
...
function areDatesInSameMonth1(date1, date2, tz) {
  const d1 = convertToTimeZone(date1, tz);
  const d2 = convertToTimeZone(date2, tz);

  return d1.getFullYear() === d2.getFullYear() && d1.getMonth() === d2.getMonth();
}
function subtractDate1(currentDate, subtract, subtractType, timezone) {
  let date = convertToTimeZone(currentDate, timezone)
...
function getDatesBetween1(startDate, endDate, repeatType, timezone,  dayOfWeek) {
...
function skipWeekday1(date, skipFrom, skipTo) {
