  ...
  return {
    next: () => {
      ...
      if(parsers.length === repeatLastDate.length && // RepeatFor entering condition
        ...
        nextDate = getNextDate(lastDate, options.repeatType)
        if(options.endDate && nextDate >= options.endDate){
          throw new Error("Out of the timespan range")
        }
        const isSameMonth = areDatesInSameMonth(lastDate, nextDate)
        if(!isSameMonth) {
          nextDate = undefined
          currentIndex = 0
        }
...
function processRepeatConfig(cronExpressions, options, endDate){
  ...
  const parsedExpressionsPrev = cronExpressions.map(expression => {
    ...
    return { expression, nextDate, nextDateStr };
  });
  // Sort the cron expressions based on the next date
  ...
  if(isCronMatched){
    ...
    if(matchedIndex > 0){
      for(let i=0; i<matchedIndex; i++){
        ...
        repeatLastDate[i] = currentInterval.next().toDate();
      }
    }
    ...
    for(let i = 1; i <= ((options.repeatFor - 1 ) * repeatParsers.length); i++){
      const lastDate = repeatLastDate[currentIndex]
      const nextDate = getNextDate(lastDate, options.repeatType) 
      if(endDate <= nextDate) {
...
function subtractDate(currentDate, subtract, subtractType) {
...
function getLastNthCronExpressions(cronExpression, options, lastDate){
  ...
  try {
    currentDate = options.currentDate.toDate()
  } catch {
    currentDate = options.currentDate
  }
  let startDate = subtractDate(lastDate, options.repeatFor-1, options.repeatType)
  if(currentDate && startDate < currentDate){
    // startDate = currentDate
  }
  if(expressionArr[2] === "L"){
    const dateArr = getDatesBetween(startDate, lastDate, options.repeatType)
  ...
  if(expressionArr[4].includes("L")){
    const dateArr = getDatesBetween(startDate, lastDate, options.repeatType, expressionArr[4].slice(0,1))
    expressionArr[2] = dateArr.join(",")
    expressionArr[4] = "?"
    return expressionArr.join(" ")
  }
...
function getDatesBetween(startDate, endDate, repeatType, dayOfWeek) {
